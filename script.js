...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  if (leftNav) {
      leftNav.style.backgroundColor = '#0000FF'; // Colore di sfondo blu per la navbar
      const links = leftNav.querySelectorAll('a');
      links.forEach(link => {
        link.style.color = '#000'; // Colore del testo nero per i link
      });
    }

    if (rightNav) {
      rightNav.style.backgroundColor = '#0000FF'; // Colore di sfondo blu per la navbar
      const links = rightNav.querySelectorAll('a');
      links.forEach(link => {
        link.style.color = '#000'; // Colore del testo nero per i link
      });
    }
